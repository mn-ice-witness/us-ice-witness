...
Important rules can get missed when buried in long documents. This project uses a two-tier documentation system:
...
- **Critical rules aren't missed** - They're prominently in CONTEXT.md
- **Easy to find the right doc** - Quick reference table maps tasks to docs
- **Detail stays accessible** - Deep info is in dev-docs, not lost
- **Smaller overhead** - CONTEXT.md is ~250 lines, not 500+

### The Contract

Reading only CONTEXT.md should be enough to:
...
1. **Frequently violated** - Commonly gotten wrong
