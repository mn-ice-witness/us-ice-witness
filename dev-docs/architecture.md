...
```
GIT_MN_ICE_FILES/
├── CONTEXT.md              # Project documentation index
...
This section describes what each JavaScript file does. Read this before modifying any JS file.
...
**Auto-generated by:** `bin/generate_summary.py`
...
### `bin/generate_summary.py` (~250 lines)
...
### `bin/process_media.py` (~427 lines)
...
2. **Processing creates new files**: `bin/process_media.py` reads from `raw_media/` and creates new compressed files in `docs/media/`.
...
2. Run `python-main bin/process_media.py` (without --force)
...
- [refactoring-plan.md](refactoring-plan.md) - Modular split recommendations
